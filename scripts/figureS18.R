### Figure S18: GLM FOR PROK CELL CARBON (FIXED EFFECT ONLY)
...
ggpubr::ggexport(figure, filename = paste("./figures/FigureS18.png", sep = ""), width = 500, height = 400)


### Save Figure S17 source data
wb <- createWorkbook(type = "xls")
sh1 <- createSheet(wb,sheetName = "Figure S18")
addDataFrame(data.frame("Figure S18"=c("Figure S18")),sheet = sh1,row.names = FALSE,startRow = 1,col.names = FALSE)
addDataFrame(fig1a_line,sheet = sh1,row.names = FALSE,startRow = 2)

sh2 <- createSheet(wb,sheetName = "Figure S18_Points")
addDataFrame(data.frame("Figure S18_Points"=c("Figure S18_Points")),sheet = sh2,row.names = FALSE,startRow = 1,col.names = FALSE)
addDataFrame(fig1a_points,sheet = sh2,row.names = FALSE,startRow = 2)

saveWorkbook(wb,"./source_data/FigureS18/FigureS18.xls")
