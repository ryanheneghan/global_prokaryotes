## Figure S9: Map of prokaryote biomass/all heterotrophs in top 200m
...
## Import data
bac_epi <- data.table::fread("./data/bacteria_epipelagic.csv")

microzoo_biom <- read.csv("./data/microzoo_predictions.csv")
macrozoo_biom <- read.csv("./data/macrozoo_predictions.csv")
mesozoo_biom <- read.csv("./data/mesozoo_predictions.csv")
rhiz_biom <- read.csv("./data/rhizaria_predictions.csv")
fish_biom <- read.csv("./data/fish_predictions.csv")
...
ggsave(plot = plot_bac, filename = "figures/FigureS9.jpeg", width = 90, height = 60, units = "mm")

write.csv(bac_frac_data, "./source_data/FigureS9/FigureS9.csv", row.names = FALSE)
