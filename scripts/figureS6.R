## FIGURE S6: global maps of SGR and GGE data sites
...
df <- read.csv("./data/raw_observations/prokaryote_sgr.csv")
df <- df %>% dplyr::mutate(log10SGR = log10(SGR),
                           log10CHL = log10(CHL),
                           log10Depth = log10(Depth)) 

gge_dat <- read.csv("./data/raw_observations/prokaryote_gge.csv")
...
ggpubr::ggexport(figure, filename = paste("./figures/FigureS6.png", sep = ""), width = 400, height =450)


### Save Figure S6 source data
dat_locs_gge <- dat_locs_gge %>% dplyr::rename(Longitude = "Long", Latitude = "Lat")

wb <- createWorkbook(type = "xls")

sh2 <- createSheet(wb,sheetName = "Figure S6a")
addDataFrame(dat_locs,sheet = sh2,row.names = FALSE,startRow = 1)

sh2 <- createSheet(wb,sheetName = "Figure S6b")
addDataFrame(dat_locs_gge,sheet = sh2,row.names = FALSE,startRow = 1)

saveWorkbook(wb,"./source_data/FigureS6.xls")
