#### FIGURE S14: Sensitivity analysis of abundance model
...
ggpubr::ggexport(figure, filename = paste("./figures/FigureS14.png", sep = ""), width = 1200, height = 1480)


### Save Figure S14 source data
wb <- createWorkbook(type = "xls")

for(i in 1:dim(save_out)[3]){
  curr_name <- dimnames(save_out)[[3]][i]
  curr_data <- as.data.frame(save_out[,,i])
  header_name <- paste("Figure S14", tagss[i], sep = "")
  ...
  addDataFrame(curr_data,sheet = sh1,row.names = FALSE,startRow = 2)
}

saveWorkbook(wb,"./source_data/FigureS14/FigureS14.xls")
