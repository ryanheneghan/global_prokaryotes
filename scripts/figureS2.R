### Figure S2: RAW RELATIONSHIP BETWEEN BACT ABUNDANCE AND FULL PREDICTOR VARIABLES
library(viridis)
library(ggplot2)
library(ggpubr)
...
df <-read.csv('./data/raw_observations/prokaryote_abundance.csv', header=TRUE)
...
ggpubr::ggexport(figure, filename = paste("./figures/FigureS2.png", sep = ""), width = 1250, height = 1470)

## Save source data
df_original <- read.csv('./data/raw_observations/prokaryote_abundance.csv', header=TRUE)
write.csv(df_original, "./source_data/FigureS2.csv", row.names = FALSE)
