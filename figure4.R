...
year_data <- read.csv("./esm_forcings/year_data_processed_epipelagic.csv")

### BIOMASS
tot_bac_biomass1 <- 1.7 # Pg wet biomass 
tot_phy_biomass1 <- 5.4 # Pg wet biomass
tot_zoo_biomass1 <- 7.8 # Pg wet biomass
tot_fish_biomass1 <- 5.5 # Pg wet biomass

#### RESPIRATION
tot_zoo_resp <- 12.4 # from Nowicki and Devries
tot_fish_resp <- 0.94 # from Bianchi et al. 2020
tot_bac_resp <- 18.5 # 18.5 Pg respiration, from data
...
plot_list[[1]] <- ggplot(year_data_gg) +
  ...
  labs(title = "", x = "Year", y = expression(bold(paste(Delta,  " Biomass (%)"))), color = "")+  
...
plot_list[[3]] <- ggplot(year_data_gg) +
  ...
  labs(title = "", x = "Year", y = expression(bold(paste(Delta,  " Respiration (%)"))), color = "")+  
...
plot_list[[4]] <- ggplot(stacked_gg) + geom_area(aes(x=Year, y = value, fill = variable)) + theme_bw() + theme_save +
  scale_x_continuous(expand = c(0,0)) + scale_y_continuous(expand = c(0,0)) + xlab("") + ylab("")+
  coord_cartesian(ylim =c(0,2), xlim = c(1980,2100))+
  labs(title = "", x = "Year", y = expression(bold(paste("Additional respiration (Pg C yr"^{-1}, ")", sep = ""))), color = "")+  
  scale_color_manual(values = colors2[c(1,3,4)], labels = c("Prokaryotes", "Zooplankton", "Fish"),aesthetics = c("colour", "fill")) +
  guides(fill = guide_legend(override.aes = list(size = 1, alpha = 1)), color = "none")+
  guides(fill = guide_legend(override.aes = list(alpha = 1)), color = "none")

figure <- ggpubr::ggarrange(plotlist = plot_list, labels = c("a", "b", "c", "d"), font.label = list(size = 8), hjust = -.5, vjust =2, align = 'hv', common.legend = TRUE,legend = "bottom", nrow = 2, ncol = 2)
ggsave(plot = figure, filename = "figures/Figure4.jpeg", width = 178, height = 150, units = "mm")




year_data <- year_data %>% dplyr::mutate(zoo_resp = zooplankton_gC*2^(temperature/10), #exp(-0.56/(8.62e-5*(temperature+273.15)
                                         fish_resp = fish_biomass_lotze*2^(temperature/10))

hist_data <- year_data %>% dplyr::filter(Year %in% 1980:1999) %>% dplyr::group_by(Model) %>% 
  dplyr::summarise(hist_bac_resp = mean(bac_resp_gC_d),
                   hist_bac_growth = mean(bac_growth_gC_d),
                   hist_bac_total = mean(bac_total_gC_d),
                   hist_zoo_resp = mean(zoo_resp),
                   hist_fish_resp = mean(fish_resp)) %>% dplyr::ungroup()

year_data2 <- left_join(year_data, hist_data, by = c("Model" = "Model")) %>% 
  dplyr::select(Year, Experiment, Model, bac_resp_gC_d, bac_growth_gC_d, bac_total_gC_d,
                hist_bac_resp, hist_bac_growth, hist_bac_total, zoo_resp, fish_resp, hist_zoo_resp, hist_fish_resp) %>%
  dplyr::mutate(bac_resp = (bac_resp_gC_d - hist_bac_resp)/hist_bac_resp*100,
                bac_growth = (bac_growth_gC_d - hist_bac_growth)/hist_bac_growth*100,
                bac_total = (bac_total_gC_d - hist_bac_total)/hist_bac_total*100,
                zoo_resp = (zoo_resp - hist_zoo_resp)/hist_zoo_resp*100,
                fish_resp = (fish_resp - hist_fish_resp)/hist_fish_resp*100,
                abs_bac_resp = tot_bac_resp*(bac_resp/100),
                abs_bac_growth = tot_bac_growth*(bac_growth/100),
                abs_bac_total = tot_bac_demand*(bac_total/100),
                abs_fish_resp = tot_fish_resp*(fish_resp/100),
                abs_zoo_resp = tot_zoo_resp*(zoo_resp/100))

year_data_gg <- year_data2 %>% dplyr::group_by(Year, Experiment) %>% 
  summarise(mean_bac_resp = mean(bac_resp),
            mean_bac_growth = mean(bac_growth),
            mean_bac_total = mean(bac_total),
            mean_zoo_resp = mean(zoo_resp),
            mean_fish_resp = mean(fish_resp),
            mean_abs_bac_resp = mean(abs_bac_resp),
            mean_abs_bac_growth = mean(abs_bac_growth),
            mean_abs_bac_total = mean(abs_bac_total),
            mean_abs_fish_resp = mean(abs_fish_resp),
            mean_abs_zoo_resp = mean(abs_zoo_resp),
            sd_bac_resp = sd(bac_resp),
            sd_bac_growth = sd(bac_growth),
            sd_bac_total = sd(bac_total),
            sd_zoo_resp = sd(zoo_resp),
            sd_fish_resp = sd(fish_resp),
            bac_resp_l = mean_bac_resp - sd_bac_resp,
            bac_resp_u = mean_bac_resp + sd_bac_resp,
            bac_growth_l = mean_bac_growth - sd_bac_growth,
            bac_growth_u = mean_bac_growth + sd_bac_growth,
            bac_total_l = mean_bac_total - sd_bac_total,
            bac_total_u = mean_bac_total + sd_bac_total,
            zoo_resp_l = mean_zoo_resp - sd_zoo_resp,
            zoo_resp_u = mean_zoo_resp + sd_zoo_resp,
            fish_resp_l = mean_fish_resp - sd_fish_resp,
            fish_resp_u = mean_fish_resp + sd_fish_resp,
...
plot_list[[1]] <- ggplot(year_data_gg) +
  geom_ribbon(aes(x = Year, ymin=bac_resp_l,ymax=bac_total_u, fill = "Prokaryotes"), alpha=0.2) +
  geom_line(aes(x = Year, y = mean_bac_resp, color = "Prokaryotes"), linewidth = 1) +
  geom_hline(yintercept = 0, color = "black", linetype = "dashed", linewidth =0.8)+ theme_bw() + theme_save +
  scale_x_continuous(expand = c(0,0)) + scale_y_continuous() + xlab("") + ylab("")+
  coord_cartesian(ylim =c(-1,12), xlim = c(1980,2100))+
  labs(title = "", x = "Year", y = expression(bold(paste(Delta,  " Carbon Demand (%)"))), color = "")+  
  scale_color_manual(values = colors,breaks = c("Prokaryotes", "Zooplankton", "Fish"),aesthetics = c("colour", "fill")) +
  guides(fill = guide_legend(override.aes = list(size = 1, alpha = 1)), color = "none")


plot_list[[2]] <- ggplot(year_data_gg) +
  geom_line(aes(x = Year, y = mean_abs_bac_resp, color = "Prokaryotes"), linewidth = 1) +
  geom_line(aes(x=Year, y = mean_abs_fish_resp, color = "Fish"), linewidth = 1) +
  geom_line(aes(x=Year, y = mean_abs_zoo_resp, color = "Zooplankton"), linewidth = 1)+
  geom_hline(yintercept = 0, color = "black", linetype = "dashed", linewidth =0.8)+ theme_bw() + theme_save +
  scale_x_continuous(expand = c(0,0)) + scale_y_continuous() + xlab("") + ylab("")+
  coord_cartesian(ylim =c(-0.15,1.45), xlim = c(1980,2100))+
  labs(subtitle = "Global Change in Prokaryote Respiration", x = "Year", y = expression(bold(paste(Delta,  " Carbon yr"^{-1}, " (Pg)"))), color = "")
