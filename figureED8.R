### Climate change projections (temperature and bacteria cell size)
...
#   geom_ribbon(aes(x = Year, ymin=chlo_l,ymax=chlo_u),  fill = "gray", alpha=0.7) +
#   geom_line(aes(x = Year, y = mean_chlo), color = "black", linewidth = 1)+
#   geom_hline(yintercept = 0, color = "black", linetype = "dashed", linewidth =0.8)+ theme_bw() + theme_save +
#   scale_x_continuous(expand = c(0,0)) + scale_y_continuous() + xlab("") + ylab("")+
#   coord_cartesian(ylim =c(-17,2), xlim = c(1980,2100))+
#   labs(title = "", x = "Year", y = expression(bold(paste(Delta,  " Chlorophyll (%)"))), color = "")
# 
# plot_list[[4]] <- ggplot(year_data_gg) +
#   geom_ribbon(aes(x = Year, ymin=nitrate_l,ymax=nitrate_u),  fill = "gray", alpha=0.7) +
#   geom_line(aes(x = Year, y = mean_nitrate), color = "black", linewidth = 1)+
#   geom_hline(yintercept = 0, color = "black", linetype = "dashed", linewidth =0.8)+ theme_bw() + theme_save +
#   scale_x_continuous(expand = c(0,0)) + scale_y_continuous() + xlab("") + ylab("")+
#   coord_cartesian(ylim =c(-7,1), xlim = c(1980,2100))+
#   labs(title = "", x = "Year", y = expression(bold(paste(Delta,  " Nitrate (%)"))), color = "")

#figure <- ggpubr::ggarrange(plotlist = plot_list, labels = c("a", "b", "c", "d"), font.label = list(size = 8), hjust = -.5, vjust =2, align = 'hv', nrow = 2, ncol = 2)
figure <- ggpubr::ggarrange(plotlist = plot_list, labels = c("a", "b"), font.label = list(size = 8), hjust = -.5, vjust =2, align = 'hv', nrow = 1, ncol = 2)
ggsave(plot = figure, filename = "figures/FigureE4.jpeg", width = 178, height = 75, units = "mm")
